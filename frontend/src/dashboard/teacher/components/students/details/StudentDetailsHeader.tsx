...
import { formatStudentDetailsDate, getStudentClassBadgeClass, getStudentStatusClass } from "./studentDetailsHelpers";
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <Badge className={getStudentClassBadgeClass(student.class)}><span className="inline-flex items-center gap-1"><GraduationCap className="h-3.5 w-3.5" />{student.class}</span></Badge>
        <Badge className={getStudentStatusClass(student.status)}>{student.status}</Badge>
