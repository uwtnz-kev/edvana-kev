  ...
  return (
    ...
    >
      <div className="flex items-center space-x-3 mb-3">
        ...
        <div>
          <h3 className="text-white font-semibold text-base transition-colors duration-300 group-hover:text-white/90">
            {title}
          </h3>
      ...
      <ul className="space-y-1 text-white/60 text-sm pl-5 list-disc">
        {bullets.map((b) => (
          <li key={b} className="transition-colors duration-300 group-hover:text-white/80">
            {b}
          </li>
        ))}
      </ul>
...
export default function TeacherChatFeatures() {
  return (
    <div className="space-y-4">
      ...
      <div
        className="
          bg-gradient-to-r from-[#1EA896]/10 to-[#FF715B]/10 border border-white/10 rounded-xl p-4
          transition-all duration-300
          hover:bg-gradient-to-r hover:from-[#1EA896]/20 hover:to-[#FF715B]/20
          hover:border-white/20
          hover:shadow-xl
          hover:-translate-y-1
        "
      >
        <div className="flex items-start gap-2">
          <BarChart3 className="h-5 w-5 text-[#1EA896] mt-0.5 flex-shrink-0 transition-transform duration-300 hover:scale-110" />
          <div>
            <h3 className="text-[#6B4F3A] font-semibold text-sm">
              Pro Tips
            </h3>
            <p className="text-[#6B4F3A] text-xs mt-1 leading-tight">
              Include subject, class level, learning objective, and difficulty.
              Ask for one output at a time (lesson plan, quiz, rubric, or marking feedback).
            </p>
          </div>
        </div>
      </div>
