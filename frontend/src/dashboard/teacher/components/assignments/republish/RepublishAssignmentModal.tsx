...
import { RotateCcw, Users, UserRoundCheck } from "lucide-react";
...
import type { RepublishAssignmentModalProps } from "./republishTypes";
      ...
      <DialogPortal>
        ...
        <DialogContent className="max-w-2xl rounded-3xl border border-white/10 bg-[#1b2430]/95 p-8 text-white shadow-[0_24px_80px_rgba(0,0,0,0.45)] backdrop-blur-xl [&>button]:hidden">
          <DialogHeader className="space-y-3 text-left">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-white/15 bg-white/10 text-white/80">
          ...
          <div className="mt-8 flex justify-end gap-3">
            ...
            <Button
              ...
              onClick={() => {
                onConfirm(mode === "class" ? { mode: "class" } : { mode: "students", studentIds: selectedStudentIds });
                onClose();
              }}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-sm font-medium text-white">
        <span className="inline-flex h-8 w-8 items-center justify-center rounded-xl border border-white/10 bg-white/10 text-white/75">
          {icon}
        </span>
        {title}
      </div>
