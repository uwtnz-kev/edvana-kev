...
import { getAssignmentDerivedStatus } from "./assignmentCardHelpers";
...
export function AssignmentCardActions({ assignment, onDelete, onDuplicate, onEdit, onPreview, onPublish, onRepublish }: Props) {
  ...
  return (
    <div className="flex flex-nowrap items-center gap-1.5">
      ...
      {derivedStatus === "closed" ? <Button type="button" onClick={onRepublish} className={`${assignmentActionButtonClass} border border-white/10 bg-white/5 text-white transition-all duration-200 hover:bg-white/20`}><RotateCcw className="h-3.5 w-3.5 text-sky-300" />Republish</Button> : null}
