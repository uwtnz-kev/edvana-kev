...
import type { AssignmentAttachment, AssignmentRepublishAudience, TeacherAssignment } from "../AssignmentsTypes";
...
function isAttachment(value: unknown): value is AssignmentAttachment {
  return isRecord(value) && typeof value.id === "string" && typeof value.name === "string" && typeof value.size === "number" && typeof value.type === "string" && typeof value.lastModified === "number" && (value.blobKey === undefined || typeof value.blobKey === "string");
}
...
function isRepublishAudience(value: unknown): value is AssignmentRepublishAudience {
  if (!isRecord(value) || typeof value.mode !== "string") return false;
  if (value.mode === "class") return true;
  return value.mode === "students" && Array.isArray(value.studentIds) && value.studentIds.every((entry) => typeof entry === "string");
}
