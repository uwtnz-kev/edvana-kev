...
import type { AssignmentPreviewStudentRecord } from "./assignmentPreviewStudentData";

type Props = {
  value: "submitted" | "not_submitted";
  submittedStudents: AssignmentPreviewStudentRecord[];
  notSubmittedStudents: AssignmentPreviewStudentRecord[];
  onChange: (value: "submitted" | "not_submitted") => void;
};

export function AssignmentPreviewSubmissionsSection({ value, submittedStudents, notSubmittedStudents, onChange }: Props) {
    ...
    <section className="rounded-2xl border border-white/20 bg-white/10 p-6 backdrop-blur-lg">
      <div className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">Switch between students who have submitted and students who still have not submitted.</p>
        </div>
        ...
        {visibleStudents.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/10 bg-white/5 px-4 py-6 text-sm text-white/60">
            {value === "submitted" ? "No submitted students in the current preview dataset." : "No students are currently marked as not submitted in the current preview dataset."}
          </div>
        ) : (
