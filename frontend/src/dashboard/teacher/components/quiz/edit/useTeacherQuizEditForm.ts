...
export function useTeacherQuizEditForm({ onSaved, quiz }: Props) {
  ...
  return {
    ...
    onOpenQuestionBuilder: () => {
      ...
      navigate(`/dashboard/teacher/questions/edit/${quiz.id}?type=quiz`, { state: { originPath: `${location.pathname}${location.search}`, originState: location.state, parentType: "quiz", mode: "edit", itemId: quiz.id } });
    },
    ...
    onSave: () => {
      if (!canSaveQuizEdit(errors)) {
        setTouched({ title: true, instructions: true, questionsText: true, dueAt: true, classId: true, classLabel: true, durationMinutes: true, totalQuestions: true, rubric: true, maxScore: true });
        scrollToFirstError();
        return;
      }
      const dueDate = new Date(values.dueAt);
      const maxScore = values.maxScore.trim().length > 0 ? Number(values.maxScore.trim()) : undefined;
      const updated = updateQuiz(quiz.id, { title: values.title.trim(), classId: values.classId, classLabel: values.classLabel, dueAt: Number.isNaN(dueDate.getTime()) ? quiz.dueAt : dueDate.toISOString(), durationMinutes: Number(values.durationMinutes), totalQuestions: Number(values.totalQuestions), instructions: values.instructions.trim(), questionsText: values.questionsText.trim() || undefined, attachments: attachments.length > 0 ? attachments : undefined, rubric: values.rubric.trim() || undefined, maxScore: typeof maxScore === "number" && Number.isFinite(maxScore) ? maxScore : undefined });
      if (updated) onSaved();
    },
