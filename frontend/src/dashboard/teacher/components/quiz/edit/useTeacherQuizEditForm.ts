...
export function useTeacherQuizEditForm({ onSaved, quiz }: Props) {
  ...
  return {
    ...
    onSubmissionMethodsChange: (_methods: SubmissionMethod[]) => {
    ...
    onSave: () => {
      ...
      if (!requiresQuestionBuilder(normalizedSubmissionMethods)) clearQuestionsForBuilder("quiz", quiz.id);
      const updated = updateQuiz(quiz.id, { title: values.title.trim(), classId: values.classId, classLabel: values.classLabel, dueAt: Number.isNaN(dueDate.getTime()) ? quiz.dueAt : dueDate.toISOString(), durationMinutes: Number(values.durationMinutes), totalAttempts: Number(values.totalAttempts), totalQuestions: questionBuilderValues.totalQuestions, submissionMethods: normalizedSubmissionMethods, instructions: values.instructions.trim(), questionsText: questionBuilderValues.questionsText, attachments: attachments.length > 0 ? attachments : undefined, rubric: values.rubric.trim() || undefined, maxScore: typeof maxScore === "number" && Number.isFinite(maxScore) ? maxScore : undefined, accessCode });
      if (updated) onSaved();
    },
