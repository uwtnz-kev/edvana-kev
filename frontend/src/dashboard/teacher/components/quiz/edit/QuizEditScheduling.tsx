  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Field label="Duration Minutes" id="quiz-edit-duration-minutes" error={state.showError("durationMinutes") ? state.errors.durationMinutes : null}>
        <Input id="quiz-edit-duration-minutes" type="number" min={1} value={state.values.durationMinutes} onChange={(event) => state.onFieldChange("durationMinutes", event.target.value)} onBlur={() => state.onFieldBlur("durationMinutes")} placeholder="20" className="h-12 w-full rounded-2xl bg-white/20 border-white/20 text-[#3B240F] placeholder:text-[#3B240F]/50" />
      </Field>
      ...
      <Field label="Class" id="quiz-edit-class-trigger" error={state.showError("classId") ? state.errors.classId : null}>
        <Select value={state.values.classId} onValueChange={state.onClassChange}>
          <SelectTrigger id="quiz-edit-class-trigger" className="h-12 w-full rounded-2xl bg-white/20 border border-white/20 text-[#3B240F] data-[placeholder]:text-[#3B240F]/50 [&>svg]:text-[#3B240F]/70"><SelectValue placeholder="Select class" /></SelectTrigger>
          <SelectContent className="bg-white/25 border border-white/25 backdrop-blur-xl rounded-2xl text-[#3B240F]">{seedClasses2.map((item) => <SelectItem key={item.id} value={item.id} className="text-[#3B240F] focus:bg-white/30 focus:text-[#3B240F]">{item.label}</SelectItem>)}</SelectContent>
        </Select>
      </Field>
      <Field label="Max Score" id="quiz-edit-max-score" error={state.showError("maxScore") ? state.errors.maxScore : null}>
        <Input id="quiz-edit-max-score" type="number" min={1} value={state.values.maxScore} onChange={(event) => state.onFieldChange("maxScore", event.target.value)} onBlur={() => state.onFieldBlur("maxScore")} placeholder="100" className="h-12 w-full rounded-2xl bg-white/20 border border-white/25 text-[#3B240F] placeholder:text-[#3B240F]/50" />
      </Field>
      <Field label="Total Questions" id="quiz-edit-total-questions" error={state.showError("totalQuestions") ? state.errors.totalQuestions : null}>
        <Input id="quiz-edit-total-questions" type="number" min={1} value={state.values.totalQuestions} onChange={(event) => state.onFieldChange("totalQuestions", event.target.value)} onBlur={() => state.onFieldBlur("totalQuestions")} placeholder="10" className="h-12 w-full rounded-2xl bg-white/20 border-white/20 text-[#3B240F] placeholder:text-[#3B240F]/50" />
      </Field>
    </div>
  );
}
