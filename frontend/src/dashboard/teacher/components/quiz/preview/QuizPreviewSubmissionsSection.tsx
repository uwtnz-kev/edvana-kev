...
import type { QuizPreviewStudentRecord } from "./quizPreviewStudentData";

type Props = {
  value: "submitted" | "not_submitted";
  submittedStudents: QuizPreviewStudentRecord[];
  notSubmittedStudents: QuizPreviewStudentRecord[];
  onChange: (value: "submitted" | "not_submitted") => void;
};

export function QuizPreviewSubmissionsSection({ value, submittedStudents, notSubmittedStudents, onChange }: Props) {
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--text-secondary)]">
            Switch between students who have submitted and students who have not submitted this quiz.
          </p>
        </div>
      ...
      <div className="mt-5 space-y-3">
        {visibleStudents.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-white/10 bg-white/5 px-4 py-6 text-center text-sm text-white/60">
            {value === "submitted" ? "No submitted students in the current quiz preview dataset." : "No students are currently marked as not submitted in the current quiz preview dataset."}
          </div>
        ) : (
