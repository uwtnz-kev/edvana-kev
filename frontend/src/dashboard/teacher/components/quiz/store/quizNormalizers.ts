...
export function normalizeQuiz(input: unknown): TeacherQuiz | null {
  ...
  if (!isRecord(input) || (input.status !== "draft" && input.status !== "published")) return null;
  ...
  return { id: input.id, title: input.title, subject: input.subject, classId, classLabel, dueAt: input.dueAt, durationMinutes, totalQuestions: typeof input.totalQuestions === "number" && Number.isFinite(input.totalQuestions) ? input.totalQuestions : 10, difficulty: normalizeDifficulty(input.difficulty) ?? "medium", type: normalizeType(input.type) ?? "mcq", status: input.status, createdAt: input.createdAt, maxScore: input.maxScore, rubric: input.rubric, instructions: input.instructions, questionsText: input.questionsText, attachments: input.attachments };
}
