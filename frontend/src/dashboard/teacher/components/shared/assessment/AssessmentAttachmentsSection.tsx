      ...
      <div className={attachmentChipStyles.list}>
        {attachments.length === 0 ? <p className="text-sm text-[var(--text-secondary)]">No files selected.</p> : attachments.map((attachment) => (
          <div key={attachment.id} className={attachmentChipStyles.item}>
            ...
            <Button type="button" onClick={() => onRemoveAttachment(attachment.id)} className="rounded-2xl border border-[var(--card-border)] bg-[var(--card-bg)] text-white hover:bg-[var(--card-hover)]">{showActionIcons ? <span className="mr-2 inline-flex h-7 w-7 items-center justify-center rounded-xl border border-slate-400/30 bg-slate-500/15 text-slate-300"><X className="h-4 w-4" /></span> : null}Remove</Button>
          </div>
        ))}
      </div>
    </div>
