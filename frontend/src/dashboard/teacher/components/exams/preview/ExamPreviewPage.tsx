...
import { ExamPreviewSubmissionsSection } from "./ExamPreviewSubmissionsSection";
import { getBaseExamSubmissionCount, getExamPreviewRoster, getExamPreviewStudentRecords } from "./examPreviewStudentData";
...
export function ExamPreviewPage({ exam, onBack }: Props) {
  const [submissionView, setSubmissionView] = useState<"submitted" | "not_submitted">("submitted");
  const roster = useMemo(() => getExamPreviewRoster(exam), [exam]);
  const baseSubmissionCount = useMemo(() => getBaseExamSubmissionCount(exam, roster.length), [exam, roster.length]);
  const totalSubmissions = useTeacherExamSubmissionTotal(exam.id, baseSubmissionCount);
  const students = useMemo(() => getExamPreviewStudentRecords(exam, totalSubmissions), [exam, totalSubmissions]);
    ...
    <div className="w-full p-4 sm:p-6">
      <div className="mx-auto max-w-5xl space-y-4">
        <header className="teacher-panel-surface rounded-2xl p-4 sm:p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-300">Exam Preview</p>
                <h1 className="text-2xl font-bold text-white sm:text-3xl">{exam.title}</h1>
                <p className="mt-1 text-sm text-white/70">Review the key exam details and current submission visibility.</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <section className="rounded-2xl border border-white/20 bg-white/10 p-6 backdrop-blur-lg">
          <ExamPreviewBody exam={exam} totalSubmissions={totalSubmissions} rosterCount={roster.length} />
        </section>

        {derivedStatus !== "draft" ? (
          <ExamPreviewSubmissionsSection
