// Renders rubric and question-builder controls used to configure exam rules and content.
import { ClipboardCheck } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import type { ExamFieldProps } from "./examCreateTypes";

type Props = ExamFieldProps & {
  onOpenPreview: () => void;
  onOpenQuestionBuilder: () => void;
};

export function TeacherExamRulesSection({
  values,
  errors,
  touched,
  onFieldChange,
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      <div className="space-y-2">
        ...
        <Textarea id="exam-rubric" value={values.rubric} onChange={(event) => onFieldChange("rubric", event.target.value)} onBlur={() => onFieldBlur("rubric")} placeholder="Scoring criteria and weight distribution" className="min-h-[110px] border-white/20 bg-white/20 text-[#3B240F] placeholder:text-[#3B240F]/50" />
      </div>

      <div className="space-y-2">
        <div className="mb-2 flex items-center justify-between">
          <label htmlFor="exam-questions-text" className="text-sm text-[#3B240F]">Questions</label>
        ...
        <Textarea id="exam-questions-text" value={values.questionsText} readOnly onBlur={() => onFieldBlur("questionsText")} placeholder="Use Input Questions to add questions" className="min-h-[140px] cursor-not-allowed border-white/15 bg-white/10 text-[#3B240F] placeholder:text-[#3B240F]/50" />
