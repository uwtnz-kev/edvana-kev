...
export function RepublishExamPage({ exam, eligibleStudents, onBackToExams, onConfirm }: Props) {
  const theme = getSubjectIconTheme(exam.subject);
  ...
  const eligibleCountLabel = useMemo(() => {
    ...
    return `${eligibleStudents.length} eligible student${eligibleStudents.length === 1 ? "" : "s"} in ${exam.classLabel}`;
  }, [exam.classLabel, eligibleStudents.length]);
      ...
      <div className="mx-auto max-w-5xl space-y-4">
        <header className="teacher-panel-surface rounded-2xl p-4 sm:p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              <Button
                type="button"
                onClick={step === "schedule" ? () => setStep("audience") : onBackToExams}
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-300">Republish Exam</p>
                <h1 className="text-2xl font-bold text-white sm:text-3xl">{step === "audience" ? "Choose audience" : "Choose close date and time"}</h1>
                <p className="mt-1 text-sm text-white/70">{exam.title}</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <section className="rounded-2xl border border-white/20 bg-white/10 p-6 backdrop-blur-lg">
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <ContextCard label="Subject" value={exam.subject} icon={<BookOpen className="h-4 w-4" />} iconClassName={theme.bgClass} iconColorClassName={theme.iconClass} />
            <ContextCard label="Class" value={exam.classLabel} icon={<GraduationCap className="h-4 w-4" />} iconClassName={theme.bgClass} iconColorClassName={theme.iconClass} />
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="mt-1 text-sm text-[var(--text-secondary)]">
                  Choose whether to reopen <span className="text-white">{exam.title}</span> for all eligible students in the class or only selected students.
                </p>
              </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  {eligibleStudents.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-white/10 bg-white/5 px-4 py-6 text-sm text-white/60">
                      There are no eligible students available for this exam yet.
                    </div>
              ...
              <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                >
                  <X className="mr-2 h-4 w-4 text-slate-300" />
                  Cancel
                </Button>
                <Button
                  type="button"
                ...
                >
                  <RotateCcw className="mr-2 h-4 w-4 text-sky-300" />
                  Continue
                </Button>
              </div>
            </div>
          ...
          <section className="rounded-2xl border border-white/20 bg-white/10 p-6 backdrop-blur-lg">
            <div className="space-y-6">
              <div>
                ...
                <p className="mt-1 text-sm text-[var(--text-secondary)]">
                  Set when the republished exam <span className="text-white">{exam.title}</span> should close for the selected audience.
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <TeacherDateTimePicker
                  ...
                  placeholder="Pick close date and time"
                />
              </div>
