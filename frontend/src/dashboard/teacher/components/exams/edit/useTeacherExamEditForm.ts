...
export function useTeacherExamEditForm({ exam, onSaved }: Props) {
  ...
  return {
    ...
    onOpenQuestionBuilder: () => {
      ...
      navigate(`/dashboard/teacher/questions/edit/${exam.id}?type=exam`, { state: { originPath: `${location.pathname}${location.search}`, originState: location.state, parentType: "exam", mode: "edit", itemId: exam.id } });
    },
    ...
    onSave: () => {
      if (!canSaveExamEdit(errors)) {
        setTouched({ title: true, instructions: true, questionsText: true, scheduledAt: true, classId: true, classLabel: true, durationMinutes: true, totalQuestions: true, rubric: true, maxScore: true });
        scrollToFirstError();
        return;
      }
