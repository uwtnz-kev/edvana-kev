import { Download, Star, Calendar, LucideIcon, Users } from "lucide-react";
    ...
    <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all duration-200 shadow-lg hover:shadow-xl">
      <div className="flex items-start space-x-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-semibold text-white mb-1 line-clamp-2">{title}</h3>
          <div className="flex items-center space-x-2 text-sm">
            <span className="px-2 py-1 bg-white/10 rounded-md text-white/80">{subject}</span>
            <span className="px-2 py-1 bg-white/10 rounded-md text-white/80">{type}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/10">
        <span className="text-white/60 text-sm font-medium">{fileSize}</span>
