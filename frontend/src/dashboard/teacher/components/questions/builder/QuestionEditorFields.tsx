...
function MultipleChoiceEditor({ question, onPatch }: Props) {
  return (
    <div className="space-y-3">
      <label className="text-sm text-[#3B240F]">Options</label>
      {question.options.map((option, index) => (
        <Input key={`${question.id}-option-${index}`} value={option} onChange={(event) => { const next = [...question.options]; next[index] = event.target.value; onPatch({ options: next }); }} placeholder={`Option ${String.fromCharCode(65 + index)}`} className="h-12 border-white/20 bg-white/20 text-[#3B240F] placeholder:text-[#3B240F]/50" />
      ))}
      <div className="space-y-2">
        <label className="text-sm text-[#3B240F]">Correct Answer</label>
        <Select value={question.correctAnswer} onValueChange={(next) => onPatch({ correctAnswer: next })}>
          <SelectTrigger className="h-12 rounded-2xl border border-white/20 bg-white/20 text-[#3B240F]"><SelectValue placeholder="Select correct answer" /></SelectTrigger>
...
function MatchingEditor({ question, onPatch }: Props) {
  return (
    <div className="space-y-3">
      <label className="text-sm text-[#3B240F]">Matching Pairs</label>
      {question.matchingPairs.map((pair, index) => (
        <div key={pair.id} className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          <Input value={pair.left} onChange={(event) => onPatch({ matchingPairs: question.matchingPairs.map((entry) => entry.id === pair.id ? { ...entry, left: event.target.value } : entry) })} placeholder={`Left item ${index + 1}`} className="h-12 border-white/20 bg-white/20 text-[#3B240F] placeholder:text-[#3B240F]/50" />
          <Input value={pair.right} onChange={(event) => onPatch({ matchingPairs: question.matchingPairs.map((entry) => entry.id === pair.id ? { ...entry, right: event.target.value } : entry) })} placeholder={`Right item ${index + 1}`} className="h-12 border-white/20 bg-white/20 text-[#3B240F] placeholder:text-[#3B240F]/50" />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-2">
      ...
      <Select value={question.trueFalseAnswer} onValueChange={(next) => onPatch({ trueFalseAnswer: next as "true" | "false" })}>
        <SelectTrigger className="h-12 rounded-2xl border border-white/20 bg-white/20 text-[#3B240F]"><SelectValue placeholder="Select true or false" /></SelectTrigger>
        <SelectContent className="rounded-2xl border border-white/20 bg-white/80 text-[#3B240F] backdrop-blur-xl">
...
function SimpleAnswerField({
  ...
  return (
    <div className="space-y-2">
      <label className="text-sm text-[#3B240F]">{label}</label>
      {multiline ? (
        <Textarea value={value} onChange={(event) => onChange(event.target.value)} placeholder="Enter expected long answer" className="min-h-[100px] border-white/20 bg-white/20 text-[#3B240F] placeholder:text-[#3B240F]/50" />
      ) : (
        <Input value={value} onChange={(event) => onChange(event.target.value)} placeholder="Enter expected answer" className="h-12 border-white/20 bg-white/20 text-[#3B240F] placeholder:text-[#3B240F]/50" />
      )}
    </div>
  );
}
