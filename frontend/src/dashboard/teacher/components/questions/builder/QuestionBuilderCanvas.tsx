// Maps the ordered question list into editable builder panels.
...
type Props = {
  ...
  onUpdate: (
    questionId: string,
    updater: (question: QuestionBuilderQuestion) => QuestionBuilderQuestion
  ) => void;
};
    ...
    <>
      {questions.map((question, index) => (
        <QuestionEditorPanel
          ...
          onMoveDown={() => onMove(question.id, 1)}
          onMoveUp={() => onMove(question.id, -1)}
          onUpdate={(updater) => onUpdate(question.id, updater)}
        />
      ))}
    </>
  );
