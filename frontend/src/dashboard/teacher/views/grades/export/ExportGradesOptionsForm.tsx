    ...
    <div className="group rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 p-4 hover:bg-white/15 transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#8B6F52]/25">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><p className="text-sm font-medium text-[#3B240F]">Grade Lists</p><p className="text-xs text-[#6B4F3A]">Pick or create a list to import into</p></div>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <Popover open={state.createOpen} onOpenChange={state.setCreateOpen}>
            <PopoverTrigger asChild>
              <button className="group h-11 px-4 rounded-2xl bg-white/10 border border-white/20 text-[#3B240F] hover:bg-white/15 transition-all duration-300 ease-out hover:scale-[1.02] hover:shadow-md hover:shadow-[#8B6F52]/20 flex items-center gap-2" type="button">
            ...
            <PopoverContent align="start" side="bottom" className="w-[--radix-popover-trigger-width] min-w-[280px] p-3 bg-white/18 backdrop-blur-xl border border-white/25 rounded-2xl">
              <div className="space-y-3">
                ...
                <Input value={state.newListName} onChange={(e) => state.setNewListName(e.target.value)} placeholder="Enter list name" className="bg-white/10 border-white/20 text-[#3B240F] placeholder:text-[#6B4F3A]" onKeyDown={(e) => { if (e.key === "Enter") state.submitCreate(); }} />
                <div className="flex items-center justify-end gap-2">
                  <button type="button" onClick={() => { state.setNewListName(""); state.setCreateOpen(false); }} className="group h-9 px-3 rounded-xl bg-white/10 border border-white/20 text-[#3B240F] hover:bg-white/15 transition-all duration-300 ease-out hover:scale-[1.02] hover:shadow-md hover:shadow-[#8B6F52]/20 text-sm font-medium">Cancel</button>
      ...
      {state.lastImportedFile ? <div className="mt-3 rounded-xl bg-white/10 border border-white/15 p-3 text-sm text-[#6B4F3A]">Imported file: <span className="font-medium text-[#3B240F]">{state.lastImportedFile}</span></div> : null}
    </div>
  );
}
