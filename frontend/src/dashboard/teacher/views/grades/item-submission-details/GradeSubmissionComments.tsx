  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex items-center gap-2"><span className="text-sm text-white/80">Total Score</span><Input type="number" min={0} max={state.submission?.maxScore} value={state.totalScore} onChange={(event) => state.setTotalScore(event.target.value)} placeholder={String(state.computedTotal)} className="w-24 bg-white/10 border-white/10 text-white placeholder:text-[var(--text-muted)]" /><span className="text-sm text-white/70">/ {state.submission?.maxScore}</span></div>
