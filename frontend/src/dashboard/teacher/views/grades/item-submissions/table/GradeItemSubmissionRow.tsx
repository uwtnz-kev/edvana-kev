...
import { toStatusClass } from "../gradeItemSubmissionsHelpers";
...
export function GradeItemSubmissionRow({ state, submission }: Props) {
  ...
  return (
    <tr className="border-t border-white/10 cursor-pointer hover:bg-white/10" onClick={() => state.openDetails(submission.id)}>
      <td className="px-4 py-3">{submission.studentName}</td><td className="px-4 py-3">{new Date(submission.submittedAt).toLocaleString()}</td><td className="px-4 py-3"><span className={`inline-flex items-center rounded-full border px-2 py-1 text-xs ${toStatusClass(submission.status)}`}>{submission.status}</span></td>
