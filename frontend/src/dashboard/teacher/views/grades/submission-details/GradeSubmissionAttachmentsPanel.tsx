  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl bg-white/10 border border-white/10 p-4">
        <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-3">Attachments</h2>
        {attachments.length > 0 ? <ul className="space-y-2">{attachments.map((item, index) => <li key={`${item.name}-${index}`} className="text-[var(--text-primary)]">{item.name}</li>)}</ul> : <p className="text-white/70">No attachments provided.</p>}
      </div>
      <div className="rounded-2xl bg-white/10 border border-white/10 p-4">
        <h2 className="text-lg font-semibold text-[var(--text-primary)] mb-3">Student Response</h2>
        <p className="text-[var(--text-primary)]">{state.submission.submissionDetails?.responseText ?? state.submission.assignmentTextResponse ?? "No text response provided."}</p>
      </div>
