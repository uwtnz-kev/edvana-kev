    ...
    <header className="teacher-panel-surface rounded-2xl px-6 py-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div><h1 className="text-2xl font-semibold text-[var(--text-primary)]">{toDetailsTitle(state.submission.type)}</h1><p className="text-[var(--text-secondary)] mt-1">{state.submission.assessmentTitle || state.submission.title}</p></div>
        </div>
      </div>
    </header>
  );
