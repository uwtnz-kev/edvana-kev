...
export function useGradeListState() {
  ...
  const createState = getCreateState(location.state); const querySubjectId = useMemo(() => new URLSearchParams(location.search).get("subjectId") ?? undefined, [location.search]); const queryType = useMemo(() => { const value = new URLSearchParams(location.search).get("type"); return value && isSelectionType(value) ? value : undefined; }, [location.search]); const workspace = useMemo(() => loadGradesWorkspace(), []);
