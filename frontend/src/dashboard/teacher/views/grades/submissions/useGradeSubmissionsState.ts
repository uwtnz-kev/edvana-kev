...
export function useGradeSubmissionsState() {
  ...
  const query = useMemo(() => new URLSearchParams(location.search), [location.search]); const queryType = query.get("type"); const querySubjectId = query.get("subjectId");
  ...
  useEffect(() => { if (!selectedGradeType) navigate(TEACHER_ROUTES.GRADES, { replace: true }); }, [navigate, selectedGradeType]);
