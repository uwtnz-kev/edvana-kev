    ...
    <header className="bg-white/10 border border-white/10 backdrop-blur-xl rounded-2xl px-6 py-5">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div><h1 className="text-2xl font-semibold text-[#3B240F]">{toSubmissionTitle(state.selectedGradeType)}</h1><p className="text-[#3B240F]/70 mt-1">{`Subject: ${state.selectedSubject?.name ?? "Not selected"}`}</p></div>
        </div>
      </div>
    </header>
  );
