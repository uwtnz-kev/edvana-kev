...
export function useExamsWorkspaceState() {
  ...
  const [search, setSearch] = useState(""); const [statusFilter, setStatusFilter] = useState<ExamStatusFilter>("all"); const [sort, setSort] = useState<ExamSort>("all"); const [page, setPage] = useState(1); const [previewId, setPreviewId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    setPreviewId(null); refreshExams(); navigate(".", { replace: true, state: null });
  ...
  const sortedExams = selectedSubjectName ? sortExams(filteredExams, sort) : [];
  ...
  return {
    canCreate, page, pagedExams, previewExam, search, selectedSubject, selectedSubjectId, sort, stats, statusFilter, subjects, totalPages,
    ...
    setSort: (value: ExamSort) => { setSort(value); setPage(1); },
