...
export default function ImportCSVModal({ open, onOpenChange, onSuccess, onClose }: ImportCSVModalProps) {
  ...
  const parseCSV = (csvText: string): CSVParent[] => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const studentIds = studentIdsStr
        ? studentIdsStr.split(",").map((id: string) => id.trim()).filter(Boolean)
        : [];
      ...
      >
        <div className="flex flex-col max-h-[85vh]">
          <DialogHeader className="flex flex-row items-center justify-between px-6 py-4 border-b border-white/20">
            <DialogTitle className="text-white text-xl font-semibold">Import Parents from CSV</DialogTitle>
            <Button
              variant="ghost"
              size="sm"
              onClick={handleClose}
            ...
            {step === "upload" ? (
              <div className="space-y-6">
                ...
                <div
                  className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors ${
                    dragActive ? "border-brand-accent bg-brand-accent/10" : "border-white/30 hover:border-white/50"
                  }`}
                  onDrop={handleDrop}
                ...
                >
                  ...
                  <div className="text-white font-medium mb-2">Drop your CSV file here, or click to browse</div>
                  <div className="text-white/60 text-sm mb-4">Supports CSV files with parent data</div>
                  <Button
                ...
                <div className="bg-white/5 rounded-xl p-4">
                  ...
                  <div className="text-white/70 text-sm space-y-1">
                    <div>
                      • <strong>firstName</strong>: 2-40 characters (required)
                    </div>
                    <div>
                      • <strong>lastName</strong>: 2-40 characters (required)
                    </div>
                    <div>
                      • <strong>email</strong>: Valid email address (required)
                    </div>
                    <div>
                      • <strong>phone</strong>: International format (required)
                    </div>
                    <div>
                      • <strong>nationalIdOrPassport</strong>: Optional
                    </div>
                    <div>
                      • <strong>status</strong>: Active or Archived (defaults to Active)
                    </div>
                    <div>
                      • <strong>studentIds</strong>: Comma-separated student IDs (optional)
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white/5 rounded-xl p-4">
                      ...
                      <tbody>
                        {csvData.slice(0, 5).map((parent, index) => {
                          ...
                          return (
                            <tr
                              key={index}
                              className={`border-b border-white/10 ${hasErrors ? "bg-red-500/5" : ""}`}
                            >
                  ...
                  {csvData.length > 5 && (
                    <div className="text-white/60 text-sm mt-2">... and {csvData.length - 5} more rows</div>
                  )}
                </div>
                ...
                <div className="flex justify-between pt-6 border-t border-white/20">
                  ...
                  <div className="flex gap-3">
                    ...
                    >
                      ...
                      {isProcessing
                        ? "Importing..."
                        : `Import ${validRowsCount} Parent${validRowsCount > 1 ? "s" : ""}`}
                    </Button>
                  </div>
                </div>
              </div>
